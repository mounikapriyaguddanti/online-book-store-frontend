...
function AdminDashboard() {
  ...
  async function fetchDataAndCreateCharts() {
    try {
      ...
      // Fetch all data
      const userStatsResponse = await fetch('http://localhost:8000/api/user-statistics');
      const userStats = await userStatsResponse.json();

      const userRegistrationsResponse = await fetch('http://localhost:8000/api/user-registrations');
      const userRegistrations = await userRegistrationsResponse.json();

      const bookStatsResponse = await fetch('http://localhost:5000/api/book-statistics');
      const bookStats = await bookStatsResponse.json();

      const publisherAuthorStatsResponse = await fetch('http://localhost:5000/api/publisher-author-statistics');
      const publisherAuthorStats = await publisherAuthorStatsResponse.json();

      const publisherPurchasesResponse = await fetch('http://localhost:5000/api/publisher-purchases');
const publisherPurchases = await publisherPurchasesResponse.json();

const userLoginCountResponse = await fetch('http://localhost:8000/api/user-login-count');
